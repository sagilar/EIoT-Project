...
static void send_message()
{
    ...
    packet_len = sprintf((char*) tx_buffer, "{mean_current:%.2f,mean_power=%.2f,max_power=%.2f,refrig_temp=%.2f,freez_temp=%.2f,door_state=%d}",
                    m_cur,m_pow,mx_pow,t_ref,t_frz,dr_stt);*/
                    
    float voltage = ((double)rand()/ (double)RAND_MAX) * 3.3;
    packet_len = sprintf((char*) tx_buffer, "{\"voltage\":%.2f}",
                    voltage);
