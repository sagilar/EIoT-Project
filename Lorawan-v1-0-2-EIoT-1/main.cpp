...
static void send_message()
{
    ...
    float t_ref = readTemperatureRefrigerator();
    float t_frz = readTemperatureFreezer();
    int dr_stt = readDoorState();
...
float readTemperatureRefrigerator()
{
    float meas_r = analog_value_temp_refrig.read(); // Read the analog input value (value from 0.0 to 1.0 = full ADC conversion range)
    float meas_v = meas_r * 3.3;
...
{
    float meas_r = analog_value_temp_freez.read(); // Read the analog input value (value from 0.0 to 1.0 = full ADC conversion range)
    float meas_v = meas_r * 3.3;
    float temperature = meas_v * 100.0; // 10 mV /°C
